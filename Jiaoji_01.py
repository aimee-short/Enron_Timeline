...
f = open('jiaoji_02.txt','w') #直接打开一个文件，如果文件不存在则创建文件
t1=time.time()
...
for t in range(1,100):
    ...
    for p in list_people[t-1]:
        ...
        #print len(p_jiaohu)
        # 下一周p出现的位置
        p_des1 = read_line(list_jiaohu1,weizhi_b,weizhi_e,int(p))#p出现的地址 第一列
        p_des2 = read_line(list_jiaohu2,weizhi_b,weizhi_e,int(p))#p出现的地址 第二列
        p_des.extend(p_des1)
        p_des.extend(p_des2)#拼接成完整地址
        for p2 in p_des:
            p_jiaohu_des.append(list_jiaohu_pinjie1[p2])
            p_jiaohu_des.append(list_jiaohu_pinjie2[p2])
